...
-?- продолжить автоматизацию теста, используя валидный код из СМС
  ...
  */
  ...
  await DSysM.androidKeyboardTypeIn('998' + phoneNumber_Receiver); // если первые цифры номера 998, то перед таким номером нужно добавить 998

  // 2.1.В поле ввода отображается введенное значение, а также - активная кнопка отправки.
  // await expect(HomeM.holderInput).toContain('998998877218'); // phoneNumber_Receiver
  await HomeM.sendButton.waitForDisplayed({timeout: GenM.waitTime + 5000});
...
*/
  ...
  await HomeM.holderInput.click();
  // 2.Ввести номер телефона получателя.
  await DSysM.androidKeyboardTypeIn('998998877218');
  ...
  */
  ...
  // 2.1.В поле ввода отображается введенное значение, а также - активная кнопка отправки.
  await expect(HomeM.holderInput).toContain('998998877218');
