...
const DialM  = require("../../screens/android/dt-dialer.screen");        // Dialer screen Model
...
const SupM   = require("../../screens/android/ab-support.screen");       // Support screen Model

describe('ab-ts-01p: Testing of operations provision | Тестирование обеспечения операций |вер.20230922| /Тестов 6 (частично 3)/', () => {
...
ШАГИ:
  ...
  */
  ...
  await expect(AuthM.supportContactsListItemWhatsApp).toBeDisplayed();
    ...
    // await expect(DDialM.dialerCallButton).toBeDisplayed();
    ...
    // // await driver.back();
    // // // /*отладка*/ console.log('\n --> phoneNumber = ' + await ADialM.dialerSearchView.getText() + '\n');
    // // await expect(DDialM.dialerSearchView).toHaveText(AuthM.supportContactphoneNumber_Expected);
...
it.only('ab-u-tc-01.003p: ! Call support | Позвонить в службу поддержки /Тест выполнен частично: нет телефонного звонка/', async () => {
  /**
...
ШАГИ:
  ...
  4.Нажать кнопку совершения телефонного вызова.
  4.1.Открыт экран устройства выполнения телефонного вызова, где доступна кнопка завершения вызова.

  5.Выполнить разговор со службой поддержки и/или завершить его, нажав кнопку завершения вызова.
  5.1.Открыт экран устройства с недавними телефонными звонками или экран устройства с контактами, а также доступна кнопка устройства Назад.

  6.Вернуться к предыдущему состоянию приложения, нажимая кнопку устройства Назад.
  6.1.Открыто окно Позвонить в банк, где доступен список телефонных номеров службы поддержки.

  7.Выполнить шаги 3-6 для каждого телефонного номера.
  *
  */
  ...
  // 1.Нажать кнопку Поддержка.
  await HomeM.bottomNavSupport.click();
  // 1.1.Открыт экран Поддержка, где доступна кнопка Позвонить в банк.
  // - экран Поддержка
  await expect(SupM.supportScreenHeader).toHaveText(SupM.supportScreenHeaderRu_Expected);

  // 2.Нажать кнопку Позвонить в банк.
  await SupM.supportCallButton.click();
  // 2.1.Открыто окно Позвонить в банк, где доступен список телефонных номеров службы поддержки.
  // - окно Позвонить в банк
  await expect(SupM.supportCallWindowLabel).toHaveText(SupM.supportCallWindowLabelRu_Expected);

  // 3.Нажать телефонный номер (любой).
  const supportPhoneNumber = await SupM.supportCallFirstContactButton.getText();
  await SupM.supportCallFirstContactButton.click();
  // 3.1.Открыт экран устройства для совершения телефонного вызова, где в поле ввода вызываемого номера отображается выбранный номер телефона, а также доступна кнопка совершения телефонного вызова.
  // - экран устройства для совершения телефонного вызова
  // -?-
  // - кнопка совершения телефонного вызова
  await expect(DialM.dialerCallButton).toBeDisplayed();
  // - номер телефона службы поддержки
  // await expect(DDialM.androidDialerphoneNumber).toBeDisplayed(); // в этом поле номер не читается, поэтому:
  // -* скрыть клавиатуру, нажав кнопку Назад и сравниваем номера:
  // await driver.back();
  // /*отладка*/ console.log('\n --> supportPhoneNumber = ' + await DialM.dialerphoneNumber.getText() + '\n');
  await expect(DialM.dialerphoneNumber).toHaveText('555120000');
  ...
  // пп.4-5 о совершении телефонного звонка автотестированию не подлежат
  // добавить вызов и сброс его
