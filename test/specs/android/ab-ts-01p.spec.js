...
describe('ab-ts-01p: Testing of operations provision | Тестирование обеспечения операций |вер.20230927| /Тестов 7 (частично 3)/', () => {
...
it('ab-u-tc-01.003p: Call support | Позвонить в службу поддержки', async () => {
  /**
...
ШАГИ:
  ...
  3.1.Открыт экран устройства для совершения телефонного вызова, где в поле ввода вызываемого номера отображается выбранный номер телефона, а также доступна кнопка совершения телефонного вызова.
    ...
    -!-1.1.1Открыт экран устройства выполнения телефонного вызова, где доступна кнопка завершения вызова.
  ...
  */
  ...
  // const raw_array = await SupM.cardsList;
  ...
  //   '\n');
  let data_array = [];
  // const elementAttributeKey = SupM.elementAttributeKey;
  const elementAttributeKey = 'resource-id';
  // const elementAttributeValue = SupM.elementAttributeValue;
  const elementAttributeValue = 'com.fincube.apexbank.debug:id/tvSupport_';
  ...
  for (let i = 0; i < data_array.length; i++) {
    ...
    supportPhoneNumber = await data_array[i].getText();
    // * Убрать из телефонного номера любые знаки, кроме цифр.
    supportPhoneNumber = await AppUM.extractNumbersFromString(supportPhoneNumber);
    supportPhoneNumber = String(supportPhoneNumber);
    ...
    // - номер телефона службы поддержки
    // /*отладка*/ console.log('\n --> DialM.supportPhoneNumber = ' + await DialM.dialerphoneNumber.getText() + '\n');
    await expect(DialM.dialerphoneNumber).toHaveText(supportPhoneNumber);
    // await expect(DialM.dialerphoneNumber).toHaveValue(supportPhoneNumber);
