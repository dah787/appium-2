...
it('? отключен выбор дизайна ab-e-tc-004p: Редактирование карты', async () => {
  /** > базовые тесты... <
...
it('?* доработать выбор карты ab-e-tc-005p: Перевод с карты на карту > Тест выполнен частично: требуется автоматически получать код из СМС <', async () => {
...
-?- продолжить автоматизацию теста, используя валидный код из СМС
  ...
  */
  ...
  // 4.1.Открыт список карт отправителя.
  // 5.Выбрать карту отправителя из списка.
  // await HCardM.senderCardSelect.click();



                                            await HCardM.senderCardsList[3].click();
  // 5.1.Закрыт список карт. В поле выбора карт отображается выбранная карта.
...
----- пропускаем проверку... с 18.05.2023 чинят */
  ...
  // 8.1.В поле ввода отображается введенное значение, кнопка Продолжить активна.
                const amountSeparatedThousandths_000 = await AppUM.separateThousandthsOfNumber(moneyAmount);
  await expect(HCardM.transferAmountInput).toHaveText(amountSeparatedThousandths);
...
it('* ab-e-tc-006p: Платеж за мобильную связь > Тест выполнен частично: требуется убрать лимит платежей <', async () => {
  /** > базовые тесты (см. файл ...) <
...
ШАГИ:
  ...
  3.Нажать кнопку статуса пользователя.
...
it('? ошибка суммы ab-e-tc-008p: Проверка баланса', async () => {
  /** > базовые тесты (см. файл ...) <
